...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> getInbox({int? userId}) async {
    ...
    try {
      ...
      response.data.forEach(
          (user) => inboxMessage!.add(ChatListResponse.fromJson(user)));
      print(userId);
      print(inboxMessage);
      print('wwwwwwwwwww');
      isGetInbox = false;
  ...
  Future<void> getMesages({int? senderId, recieveId}) async {
    ...
    try {
      var response = await DioHelper.getData(
        url:
            '${AppApiPaths.base}/api/v1/messenger/Conversation?SenderId=$senderId&receiverId=$recieveId',
      );
  ...
  Future<void> readMesages(String? hash) async {
    isSentMessage = true;
    notifyListeners();
